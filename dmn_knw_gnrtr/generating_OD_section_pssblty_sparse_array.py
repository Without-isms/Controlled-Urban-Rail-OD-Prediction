...
def generating_OD_section_pssblty_sparse_array(base_dir, station_manager_dict_name,
    ...
    def processing_Time_DepartFreDic_item(Time_DepartFreDic_item):
        ...
        if not Time_DepartFre_value:
            ...
            sparse_tensor = torch.sparse_coo_tensor(indices=torch.empty((5, 0), dtype=torch.int32),
                                                    ...
                                                    size=sparse_shape)
            sparse_5d_tensor = [sparse_tensor]
        else:
            tensors_all_OD = torch.zeros((num_stations, num_stations, 3, num_stations, num_stations))
            if Use_3D_transform:
                ...
                for k in range(3):
                    ...
                    if Use_3D_transform:
                        sparse_tensor = torch.sparse_coo_tensor(indices, values,
                                                                (num_stations, num_stations)).coalesce()
        ...
        return date_and_time, OD_feature_array, sparse_5d_tensor

    if Parallel_or_not:
        with ProcessPoolExecutor(max_workers=60) as executor:
            results = list(tqdm(executor.map(processing_Time_DepartFreDic_item, Time_DepartFreDic.items()),
                                total=len(Time_DepartFreDic)))

        for date_and_time, OD_feature_array, sparse_5d_tensor in results:
        ...
        for Time_DepartFreDic_item in tqdm(Time_DepartFreDic.items(), desc="Processing items"):
            Time_DepartFre_key, Time_DepartFre_value = Time_DepartFreDic_item
            date_and_time = Time_DepartFre_key
            date_and_time, OD_feature_array, sparse_5d_tensor=processing_Time_DepartFreDic_item(Time_DepartFreDic_item)
            OD_feature_array_dic[date_and_time] = OD_feature_array
            Date_and_time_OD_path_dic[date_and_time] = sparse_5d_tensor
...
Date_and_time_OD_path_dic_file_path = os.path.join(base_dir,
                                                   f'Date_and_time_OD_path_dic.pkl')
generating_OD_section_pssblty_sparse_array(base_dir, station_manager_dict_name,
                                               Time_DepartFreDic_file_path, OD_path_dic_file_path,
                                               station_manager_dict_file_path, OD_feature_array_file_path,
                                               Date_and_time_OD_path_dic_file_path)'''