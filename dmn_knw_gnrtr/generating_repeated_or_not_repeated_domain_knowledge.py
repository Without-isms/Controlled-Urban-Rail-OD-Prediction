...
def generating_repeated_or_not_repeated_domain_knowledge(base_dir, prefix, repeated_or_not_repeated, seq_len):
    ...
    if repeated_or_not_repeated=="not_repeated":
        ...
        """sparse_tensors_2D = torch.load(os.path.join(base_dir, f'{prefix}_sparse_tensors_OD_3_paths_all_OD.pt'))
        sparse_tensors_3D = torch.load(os.path.join(base_dir, f'{prefix}_sparse_3d_tensor_list.pt'))
        sparse_tensors_4D = torch.load(os.path.join(base_dir, f'{prefix}_sparse_4d_tensor_list.pt'))"""
        ...
        """output_file_path_2D = open(os.path.join(base_dir, f'{prefix}_repeated_sparse_2D_tensors.pt'), 'wb')
        torch.save(repeated_2D_OD_path_lst_, output_file_path_2D)

        output_file_path_3D = open(os.path.join(base_dir, f'{prefix}_repeated_sparse_3D_tensors.pt'),'wb')
        torch.save(repeated_3D_OD_path_lst_, output_file_path_3D)

        output_file_path_4D = open(os.path.join(base_dir, f'{prefix}_repeated_sparse_4D_tensors.pt'),'wb')
